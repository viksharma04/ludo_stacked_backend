    ...
    async def join_room(self, user_id: str, room_code: str) -> JoinRoomResult:
        ...
        try:
            ...
            if existing_seat:
                ...
                snapshot = await self.get_room_snapshot(room_id)
                logger.info("User %s rejoined room %s at seat %d", user_id, room_id, existing_seat.seat_index)
                return JoinRoomResult(
                    success=True,
            ...
            logger.exception("Error joining room with code %s for user %s: %s", room_code, user_id, e)
            return JoinRoomResult(
                success=False,
                error_code="INTERNAL_ERROR",
    ...
    def _update_seat_in_db(self, room_id: str, seat_index: int, user_id: str) -> bool:
        ...
        try:
            ...
            if response.data and len(response.data) > 0:
                return True
            logger.warning("Seat %d in room %s was not updated (possibly taken)", seat_index, room_id)
            return False
        except Exception as e:
